...
public class AutoSetUserAdapterFilter implements Filter {
	...
	private final static String LOGIN_SESSION_ID ="loginCustomer";
	private final static String LOGIN_MESSAGE_INFO = "loginLaboratorys";
	@Override
	public void destroy() {
		...
		if (!Arrays.asList(roleIds).contains("99")) {
			if (attributes != null && attributes.size() > 0) {
				...
				if(StringUtils.isEmpty(customer)) {
					 ...
					 customer.setCustOid(getUUID() );
					 customer.setCustId(Integer.valueOf(no));
					 ...
					 customer.setCustDesc("初始化登录用户名:"+loginname);
